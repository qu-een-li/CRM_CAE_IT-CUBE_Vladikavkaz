...
def generate_school_name():
    ...
    specializations = [
        "программирования", "дизайна", "языков", "бизнеса", "творчества"
    ]
...
for i in range(N_OF_GROUPS):
    ...
    group.name_of_group = choice(
        ['python на ура', 'Не потерянные', 'змейка', 'восставшие', 'киберлюди', 'технологи'])
    group.teacher = choice(generated_teachers)
    group.description = f'Обучаем {direction.name}'
    group.level_of_education = choice(
        ["вводный", "базовый", "углубленный", "проектный"])
