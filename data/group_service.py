...
class GroupService:
    """Класс с методами для создания группы."""
    ...
    @staticmethod
    def generate_semester_lessons(
        first_lesson_date: date, start_time: time, end_time: time, add_days: list, period_start: date, period_end: date
    ) -> list:
    ...
    def create_semester_group(
        ...
        lessons = GroupService.generate_semester_lessons(
            first_lesson_date=first_date,
            start_time=start,
            end_time=end,
            add_days=add_days,
            period_start=study_period.date_start,
            period_end=study_period.date_end,
        )

        for lesson in lessons:
            if not GroupService.auditorium_free(
                session, auditorium_id, lesson["date"], lesson["start_time"], lesson["end_time"]
            ):
        ...
        group = Group(
            ...
            group_type="семестровый",
            level_of_group=level_of_group,
    ...
    def create_intensive_group(
        ...
        for lesson in custom_lessons:
            schedule = Schedule(
                group_id=group.id,
                date=datetime.strptime(lesson["date"], "%Y-%m-%d").date(),
